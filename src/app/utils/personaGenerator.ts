...
interface PersonaData {
  ...
  nurtureText: string;
  // Editable fields (calculated but can be overridden)
...
export function generatePlexusPersona(specifiedCustomerType?: string, specifiedLoyalty?: string, specifiedGeneration?: string, overrideText?: string): PersonaData {
  ...
  return {
    ...
    // Editable fields (calculated but can be overridden)
    ambassadorRank: targetScore < 35 ? "Retail" : targetScore <= 75 ? "VIP" : "Ambassador",
    previousVIPStatus: targetScore < 35 ? "Self Discovery" : targetScore <= 75 ? "Through my Network" : "Through ambassador",
    potentialBusinessInterest: (() => {
      // Parse income to calculate business interest
    ...
    revenueOpportunity: (() => {
      // Parse income to calculate revenue opportunity
      const match = income.match(/\$?([\d,]+)/);
      if (!match) return "N/A";
      const incomeValue = parseInt(match[1].replace(/,/g, ''), 10);
      if (isNaN(incomeValue)) return "N/A";
      if (incomeValue < 20000) return "Low";
      if (incomeValue >= 20000 && incomeValue <= 34000) return "Moderate";
      if (incomeValue >= 35000 && incomeValue <= 49000) return "High";
      if (incomeValue >= 50000 && incomeValue <= 74000) return "Very High";
      if (incomeValue >= 75000 && incomeValue <= 99000) return "Extremely High";
      if (incomeValue >= 100000) return "Outstanding";
      return "N/A";
    })()
  };
}